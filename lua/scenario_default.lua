...
on_event("prestart", function()
	...
	if #leaders < 2 then
		...
		create_timed_spawns(5, 11, 50, 5, 3, 21)
	else
		--create_timed_spawns(4, 11, 90, 4, 5, 23)
		create_timed_spawns(5, 11, 90, 4, 4, 23)
	end
end)
...
on_event("side 1 turn", function()
	...
	if weather_event.weather_id == "clear" then
		weather_map("~add-ons/Isle_of_Mists/maps/2p_Isle_of_Mists_basic.map")
	...
	elseif weather_event.weather_id == "drought" then
		weather_map ("~add-ons/Isle_of_Mists/maps/2p_Isle_of_Mists_drought.map")
		wesnoth.wml_actions.sound {
			name = "gryphon-shriek-1.ogg",
		}
		weather_alert(_"Drought", 251, 231, 171)
	elseif weather_event.weather_id == "heavy rain" then
		weather_map("~add-ons/Isle_of_Mists/maps/2p_Isle_of_Mists_rain.map")
		wesnoth.wml_actions.sound {
	...
	elseif weather_event.weather_id == "heavy snowfall" then
		weather_map("~add-ons/Isle_of_Mists/maps/2p_Isle_of_Mists_secondsnow.map")
		wesnoth.wml_actions.sound {
